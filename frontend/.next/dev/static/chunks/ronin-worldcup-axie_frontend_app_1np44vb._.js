...
function getCountryTokenId(country) {
    ...
    const index = __TURBOPACK__imported__module__$5b$project$5d2f$ronin$2d$worldcup$2d$axie$2f$frontend$2f$app$2f$lib$2f$countries$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["countries"].findIndex((c)=>c.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "") === normalizedCountry.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""));
            ...
            async function loadWinnerOdds() {
                try {
                    ...
                    const rows = (event.markets || []).map({
                        "OddsPage.useEffect.loadWinnerOdds.rows": (market)=>{
                            ...
                            const yesIndex = outcomes.findIndex({
                                "OddsPage.useEffect.loadWinnerOdds.rows.yesIndex": (o)=>o === "Yes"
                            }["OddsPage.useEffect.loadWinnerOdds.rows.yesIndex"]);
                            if (yesIndex === -1) {
                                return null;
                            }
                            return {
                    ...
                    }["OddsPage.useEffect.loadWinnerOdds.rows"]).filter(Boolean).sort({
                        "OddsPage.useEffect.loadWinnerOdds.rows": (a, b)=>b.probability - a.probability
                    }["OddsPage.useEffect.loadWinnerOdds.rows"]);
                    setRows(rows);
                    setStatus("");
                } catch (err) {
        ...
        "OddsPage.useEffect": ()=>{
            async function loadTopScorerCountry() {
                try {
                    ...
                    const event = await res.json();
                    const rows = (event.markets || []).map({
                        "OddsPage.useEffect.loadTopScorerCountry.rows": (market)=>{
                            if (!market.outcomes || !market.outcomePrices) {
                                return null;
                            }
                            let outcomes;
                            ...
                            const yesIndex = outcomes.findIndex({
                                "OddsPage.useEffect.loadTopScorerCountry.rows.yesIndex": (o)=>o === "Yes"
                            }["OddsPage.useEffect.loadTopScorerCountry.rows.yesIndex"]);
                            if (yesIndex === -1) {
                                return null;
                            }
                            return {
                    ...
                    }["OddsPage.useEffect.loadTopScorerCountry.rows"]).filter({
                        "OddsPage.useEffect.loadTopScorerCountry.rows": (row)=>row && row.country && !Number.isNaN(row.probability)
                    }["OddsPage.useEffect.loadTopScorerCountry.rows"]).sort({
                        "OddsPage.useEffect.loadTopScorerCountry.rows": (a, b)=>b.probability - a.probability
                    }["OddsPage.useEffect.loadTopScorerCountry.rows"]);
            ...
            async function loadTopScoringNation() {
                try {
                    ...
                    const rows = markets.map({
                        "OddsPage.useEffect.loadTopScoringNation.rows": (market)=>{
                            if (!market.outcomes || !market.outcomePrices) {
                                return null;
                            }
                            const outcomes = JSON.parse(market.outcomes);
                            const prices = JSON.parse(market.outcomePrices);
                            const yesIndex = outcomes.findIndex({
                                "OddsPage.useEffect.loadTopScoringNation.rows.yesIndex": (o)=>o === "Yes"
                            }["OddsPage.useEffect.loadTopScoringNation.rows.yesIndex"]);
                    ...
                    }["OddsPage.useEffect.loadTopScoringNation.rows"]).filter(Boolean).sort({
                        "OddsPage.useEffect.loadTopScoringNation.rows": (a, b)=>b.probability - a.probability
                    }["OddsPage.useEffect.loadTopScoringNation.rows"]);
...
_s(OddsPage, "toUY9c/IwvgiCroWjcKbQwhst1g=", false, function() {
    return [
...
//# sourceMappingURL=ronin-worldcup-axie_frontend_app_1np44vb._.js.map