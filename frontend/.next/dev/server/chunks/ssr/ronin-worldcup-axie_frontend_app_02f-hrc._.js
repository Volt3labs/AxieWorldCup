...
function LeaderboardPage() {
    ...
    async function loadLeaderboard() {
        try {
            ...
            setStatus(`Indexed to block ${Number(data.lastIndexedBlock).toLocaleString()} · collectors: ${data.collectors?.length || 0} · mints: ${data.latestMints?.length || 0}`);
            setStatus(`Indexed to block ${Number(data.lastIndexedBlock).toLocaleString()}`);
        } catch (err) {
